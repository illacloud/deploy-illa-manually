...
## copy backend bin
COPY --from=builder-for-backend /opt/illa/builder-backend/bin/illa-backend-ws /opt/illa/builder-backend/bin/
...
# run
EXPOSE 8000
CMD ["/bin/sh", "-c", "/opt/illa/builder-backend/bin/illa-backend-ws"]
